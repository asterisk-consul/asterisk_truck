  ...
  async () => {
    ...
    try {
      const [detail, movements] = await Promise.all([
        // ✅ método correcto
        SalesService.getProductById(productId.value),
      ...
      // fallback
      const movements = await SalesService.getMovements({
        ...filters,
        productId: productId.value
      })
        ...
        <div class="bg-white dark:bg-gray-900 p-4 rounded-lg border">
          ...
          <table class="w-full text-xs">
            ...
            <tbody class="divide-y divide-gray-200">
              ...
              <tr v-for="m in data?.movements || []" :key="m.ref + m.number">
                <td class="py-2">{{ fmtDate(m.date) }}</td>
                <td class="py-2">{{ m.documentTypeName }}</td>
                <td class="py-2">{{ m.supplierName }}</td>
