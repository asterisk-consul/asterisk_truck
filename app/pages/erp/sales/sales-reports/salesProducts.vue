...
} from '~/modulos/erp/sales/types/salesReports.types'
...
const data = ref<SalesSummaryResponse | null>(null)
...
const search = ref('')
const sortCol = ref<string>('totalSales')
...
const maxAbs = computed(() =>
  Math.max(...filtered.value.map((p) => Math.abs(p.totalSales) || 0), 1)
)
...
async function renderCharts() {
  ...
  if (barChartRef.value) {
    barChartInstance = new Chart(barChartRef.value, {
      ...
      data: {
        ...
        datasets: [
          {
            data: top8.map((p) => Math.round(p.totalSales)),
  ...
  const otherSum = posProds.slice(5).reduce((a, b) => a + b.totalSales, 0)
  const dData = [
    ...top5.map((p) => Math.round(p.totalSales)),
    Math.round(otherSum)
  ]
              ...
              <thead>
                ...
                >
                  ...
                  <th
                    class="th tr"
                    style="width: 140px"
                    @click="setSort('totalSales')"
                  >
                    Total gravado
                    <span
                      :class="['sort', sortCol === 'totalSales' && 'active']"
                    >
                      {{ sortIcon('totalSales') }}
                    </span>
                  </th>
              ...
              <tbody>
                ...
                >
                  ...
                  <td
                    ...
                    :class="p.totalSales < 0 ? 'text-red-500' : ''"
                  >
                    {{ fmt(p.totalSales) }}
                  </td>
                    ...
                    <div class="flex items-center gap-1">
                      ...
                      >
                        <div
                          ...
                          :style="{
                            width:
                              ((Math.abs(p.totalSales) / maxAbs) * 100).toFixed(
                                1
                              ) + '%',
                            background: p.totalSales < 0 ? '#ef4444' : '#185FA5'
                          }"
                        />
                      </div>
                      <span class="tabular-nums text-gray-400 w-6 text-right">
                        {{ ((p.totalSales / maxAbs) * 100).toFixed(0) }}%
                      </span>
                    </div>
                  </td>
                </tr>
